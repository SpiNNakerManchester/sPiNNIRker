...
static inline void do_spike(spike_t spike, conv1d_spike_data_t *spike_data,
        conv1d_data_t *conv1d_data, int32_t *out_data) {
    ...
    for (int32_t w = -conv1d_data->half_conv_size;
        ...
        if (is_conv_1d_output(i_idx, w, conv1d_data, &out_i)) {
            ...
            for (uint32_t och = 0; och < conv1d_data->output_channels; och++) {
                ...
                out_data[o_idx + out_i] +=
                    conv1d_data->weights[w_idx + w + conv1d_data->half_conv_size];
            }
        }
    }
}
...
static void conv1d_spikes_exec(void *data, uint32_t n_inputs, data_t *input,
    ...
    for (uint32_t inp = 0; inp < n_inputs; inp++) {
        ...
        for (uint32_t i = 0; i < in_data->n_spikes; i++) {
            ...
            do_spike(spike, spike_data, conv1d_data, out_data);
        }
    }
}
