...
*/
//! \file leaky_integrate_and_fire_matrix.c
...
#include <spin1_api.h>
#include <debug.h>
#include "leaky_integrate_and_fire_matrix.h"
...
} leaky_integrate_and_fire_matrix_config_t;

typedef struct {
    uint32_t key;
    matrix_data_t matrix_data;
} leaky_integrate_and_fire_matrix_data_t;

static void *leaky_integrate_and_fire_matrix_init(uint32_t index, void *params) {
    leaky_integrate_and_fire_matrix_data_t *data = spin1_malloc(
            sizeof(leaky_integrate_and_fire_matrix_data_t));
    if (!data) {
        log_error("Failed to allocate leaky_integrate_and_fire matrix data structure");
        return (void *)0;
    }
    leaky_integrate_and_fire_matrix_config_t *config = params;
    data->key = config->key;
...
static void leaky_integrate_and_fire_matrix_exec(void *data, uint32_t n_inputs,
            data_t *input, data_t output) {
    // Get the data structure
    leaky_integrate_and_fire_matrix_data_t *lif_data = data;
...
const component_t leaky_integrate_and_fire_matrix = {
        .init = leaky_integrate_and_fire_matrix_init,
        .func = leaky_integrate_and_fire_matrix_exec,
