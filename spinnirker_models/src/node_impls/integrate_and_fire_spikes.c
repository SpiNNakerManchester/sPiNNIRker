...
*/
//! \file integrate_and_fire_spikes.c
//! \brief integrate_and_fire NIR component implementation (spike inputs)

#include <arm_acle.h>
#include <stdfix-full-iso.h>
#include <spin1_api.h>
#include <debug.h>
#include "integrate_and_fire_spikes.h"
...
} integrate_and_fire_spikes_item_t;
...
} integrate_and_fire_spikes_config_t;
...
} integrate_and_fire_spikes_data_t;

static void *integrate_and_fire_spikes_init(uint32_t index, void *params) {
    integrate_and_fire_spikes_data_t *data = spin1_malloc(
            sizeof(integrate_and_fire_spikes_data_t));
    if (!data) {
        log_error("Failed to allocate leaky_integrate_and_fire spikes data structure");
        return (void *)0;
    }
    integrate_and_fire_spikes_config_t *config = params;
    data->input_width_inv = config->input_width_inv;
    data->key = config->key;
    return matrix_init(index, &config->matrix_data, &data->matrix_data,
        sizeof(integrate_and_fire_spikes_item_t));
}

static void integrate_and_fire_spikes_exec(void *data, uint32_t n_inputs,
            data_t *input, data_t output) {
    // Get the data structure
    integrate_and_fire_spikes_data_t *lif_data = data;
    ...
    while (matrix_spikes_loop_is_next(&loop, &row, &col)) {
        integrate_and_fire_spikes_item_t *row_data = loop.current_data;
...
const component_t integrate_and_fire_spikes = {
        .init = integrate_and_fire_spikes_init,
        .func = integrate_and_fire_spikes_exec,
